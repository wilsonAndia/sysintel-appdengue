...
type GroupDetailRouteProp = RouteProp<RootStackParamList, 'ZoneDetail'>;
...
const ZoneDetail = () => {
    ...
    try {
      ...
      if (response.statusCode === 200) {
        ...
        setZone({
          ...
          longitude: zoneResponse.longitude,
          latitude: zoneResponse.latitude,
        });
  ...
  const generateSquarePolygon = (center: LatLng, distance: number = 0.005) => {
    ...
    return [
      {
        latitude: Number(center.latitude) + distance,
        longitude: Number(center.longitude) - distance,
      },
      {
        latitude: Number(center.latitude) + distance,
        longitude: Number(center.longitude) + distance,
  ...
  const handleSaveZone = async () => {
    ...
    } else if (zone.latitude === 0 || zone.longitude === 0) {
          ...
          <View style={{flex: 1, justifyContent: 'flex-start'}}>
            ...
            <View style={tw`h-[400px] relative mt-14`}>
              ...
              <MapView
                ...
                style={{flex: 1}}
                initialRegion={{
                  latitude: Number(zone.latitude), // Evita undefined
